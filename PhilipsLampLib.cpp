...
void PhilipsLampLib::listening(uint8_t duration, std::function<void(uint8_t*)> callback) {
    uint8_t result = 0;
...
void PhilipsLampLib::sendBurstCommand(uint8_t command, uint8_t* data, uint8_t length) {
    digitalWrite(SPI_CS, LOW);
    delayMicroseconds(1);
    SPI.write(command);
    delayMicroseconds(1);
